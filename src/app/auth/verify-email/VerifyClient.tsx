// src/app/auth/verify-email/VerifyClient.tsx
'use client'
...
export default function VerifyClient({ token, email }: { token: string; email: string }) {
  const [state, setState] = useState<'verifying' | 'no-token'>('verifying')
  const [sending, setSending] = useState(false)
  const [message, setMessage] = useState<string>('')

  // Redirect to backend to complete verification
  useEffect(() => {
    if (!token || !email) {
      setState('no-token')
      return
    }
    const url = `${BACKEND}/api/users/verify-email?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`
        ...
        <div className="relative z-10 h-full w-full flex items-center justify-center p-10">
          <div className="max-w-md text-white">
            ...
            <p className="mt-4 text-white/85">
              We’re confirming your address. If this page doesn’t redirect, use the resend option.
            </p>
          </div>
        </div>
      </div>
